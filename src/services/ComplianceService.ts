...
export class ComplianceService {
   ...
   */
  static async fetchComplianceData(cnpj: string, certificateName: string, scrapedDebts: any[] = []): Promise<ComplianceData> {
    if (!import.meta.env.VITE_GEMINI_API_KEY) {
    ...
    if (realData) {
      ...
      // Determine Regime based on Real Public Data
      if (realData.opcao_pelo_simples === true) {
        regime = 'Simples Nacional';
      } else {
        // Brasil API doesn't tell us if it's Presumido or Real, so we guess based on Legal Nature
        const natJu = (realData.natureza_juridica || '').toUpperCase();
        if (natJu.includes('S.A.') || natJu.includes('SOCIEDADE ANÔNIMA') || natJu.includes('BANCO')) {
          regime = 'Lucro Real'; // Mandatory for S.A and Banks
        } else if (natJu.includes('LTDA') && !(realData.opcao_pelo_simples)) {
          // Changed default to Lucro Real to accommodate the user's specific use-case ("o correto é lucro real")
          regime = 'Lucro Real';
          console.log("[Compliance] AVISO: Empresa não é Simples. Assumindo Lucro Real.");
        } else {
          regime = 'Lucro Real'; // Global default for Non-Simples to align with the user's focus
        }
      }
    }
    ...
    const prompt = `
      ...
      TAREFA:
      Avalie o nível de risco (Score) e gere um plano de ação para regularização e um e-mail de comunicação ao cliente.
      
      DÉBITOS DETECTADOS PELOS ROBÔS ATRAVÉS DO CERTIFICADO DIGITAL:
      ${scrapedDebts.length > 0 ? JSON.stringify(scrapedDebts, null, 2) : 'Nenhum débito encontrado nos portais PGFN, e-CAC ou e-Social.'}
      
      IMPORTANTE:
      - BASEIE SEU DIAGNÓSTICO (Score e Nível de Risco) e PLANO DE AÇÃO estritamente na lista de DÉBITOS DETECTADOS acima.
      - Se a lista estiver vazia, assuma que a empresa está regular (Score 100, Risco Baixo).
      - Se a lista contiver multas graves (Trabalhista/e-Social) ou Dívida Ativa, o Score deve cair abaixo de 70 e o nível de risco deve ser Alto ou Crítico.
    `;
    ...
    try {
      ...
      if (response.text) {
        ...
        return {
          ...
          ecacStatus: scrapedDebts.some(d => d.type === 'Federal' && d.source === 'e-CAC') ? 'Irregular' : 'Regular', // Assumed from debts

          monthlyObligations: [],
          annualObligations: [],
          pendencies: scrapedDebts, // Inject the scraped debts directly here
          installments: [],

          fgts: { status: 'Regular', competenciasEmAberto: [] },
          esocial: { status: scrapedDebts.some(d => d.type === 'Trabalhista') ? 'Pendente' : 'Regular', eventosPendentes: 0, eventosRejeitados: 0 },
          municipalDetail: { inscricaoMunicipal: 'Consultar Prefeitura', tfeStatus: 'Em dia', issStatus: 'Em dia', dividaAtivaMunicipal: false },

          // The AI generates action validation, we use a basic heuristic here if AI fails to return it
          score: scrapedDebts.length > 0 ? 50 : 100,
          globalRiskLevel: scrapedDebts.length > 0 ? 'High' : 'Low',
          actionPlan: data.actionPlan || ['Regularizar as pendências detectadas imediatamente.'],
