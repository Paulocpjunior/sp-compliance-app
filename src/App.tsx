...
const App: React.FC = () => {
    ...
    const handleUnlock = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 1. Run Backend Scraping Robots FIRST
            const scrapedDebts: any[] = [];
            const pushPendency = (item: any, source: string, riskLevel: 'Critical' | 'High' | 'Medium' | 'Low' = 'High') => {
                scrapedDebts.push({
                    description: item.descricao || item.tipo || item.description || `Dívida Encontrada (${source})`,
                    amount: item.valor || item.amount || 0,
                    riskLevel: item.risco || item.riskLevel || riskLevel,
                    type: source as any,
                    status: item.status || 'Pendente',
                    source: item.source || source // useful for ecac tracking
                });
            };
            ...
            try {
                [ecacData, pgfnData, pgeData, esocialData] = await Promise.allSettled([
                    BackendService.auditEcac(file, password),
                    BackendService.auditPgfn(file, password),
                    BackendService.auditPge(file, password),
                    BackendService.auditEsocial(file, password)
                ]);

                // 🚨 CRITICAL FIX: explicit check for 401 Rejections that are absorbed by allSettled
                const checkFatalError = (res: any, portal: string) => {
                    if (res.status === 'rejected') {
                        const msg = res.reason?.message || String(res.reason);
                        if (msg.includes('401') || msg.includes('Autorizado') || msg.includes('ACESSO NEGADO')) {
                            throw new Error(`O Portal ${portal} rejeitou o Certificado Digital (401 Acesso Negado). O CNPJ não tem permissão ou a cadeia V3 do certificado está incompleta.`);
                        }
                        console.error(`Scraping falhou silenciosamente para ${portal}:`, msg);
                    }
                };

                checkFatalError(ecacData, 'e-CAC');
                checkFatalError(pgfnData, 'PGFN');
                checkFatalError(esocialData, 'e-Social');

                if (ecacData.status === 'fulfilled' && ecacData.value?.pendencies) {
                    ecacData.value.pendencies.forEach((p: any) => pushPendency(p, 'Federal', p.risco || 'High'));
                }

                if (pgfnData.status === 'fulfilled' && pgfnData.value?.debts) {
                    pgfnData.value.debts.forEach((d: any) => pushPendency(d, 'Federal', 'Critical'));
                }

                if (pgeData.status === 'fulfilled' && pgeData.value?.debts) {
                    pgeData.value.debts.forEach((d: any) => pushPendency(d, 'State', 'Critical'));
                }

                if (esocialData.status === 'fulfilled' && esocialData.value?.pendencies) {
                    esocialData.value.pendencies.forEach((p: any) => pushPendency(p, 'Trabalhista', 'High'));
                }
            } catch (e: any) {
                console.error("Erro fatal capturado durante a execução dos robôs:", e);
                // We MUST re-throw the 401/Gov.br error so the entire process aborts
                // and the user sees the red warning banner, skipping the AI report.
                if (e?.message && (e.message.includes('401') || e.message.includes('Acesso') || e.message.includes('Autorizado') || e.message.includes('rejeitou') || e.message.includes('Erro reportado'))) {
                    throw e;
                }
            }

            // 2. NOW fetch Compliance Data AND generate the AI Report using the REAL scraped debts
            try {
                const finalComplianceData = await ComplianceService.fetchComplianceData(parsed.cnpj || '', subjectName, scrapedDebts);

                // Extra UI tweaks based on rules
                if (pgfnData?.status === 'fulfilled' && pgfnData.value?.debts?.length > 0) {
                    finalComplianceData.cndFederalStatus = 'Com Pendências';
                }
                if (pgeData?.status === 'fulfilled' && pgeData.value?.debts?.length > 0) {
                    finalComplianceData.cndStateStatus = 'Com Pendências';
                }

                setComplianceData(finalComplianceData);
            } catch (err) {
                console.error("Compliance Data Error", err);
                setError("Erro ao gerar o relatório consolidado de Compliance.");
            } finally {
        ...
        <Layout user={user} onLogout={handleLogout}>
            ...
            <div className="flex space-x-1 mb-8 bg-white dark:bg-slate-800 p-1 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 inline-flex transition-colors">
                <button
                    ...
                    className={`px-5 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 ${activeTab === 'analysis'
                ...
                <button
                    ...
                    className={`px-5 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 ${activeTab === 'dashboard'
                                                ...
                                                <div className="flex items-start justify-between">
                                                    ...
                                                    {certData.isICPBrasil && (
                                                        <div className="flex flex-col items-end">
                                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-green-600 mb-1" viewBox="0 0 24 24" fill="currentColor">
                                                                <path fillRule="evenodd" d="M10.496 2.132a1 1 0 00-.992 0l-7 4A1 1 0 002 7v5a11 11 0 0010 10.462A11 11 0 0022 12V7a1 1 0 00-.504-.868l-7-4zM16.38 8.35a1 1 0 00-1.42 0L10 13.28l-2.96-2.93a1 1 0 00-1.42 1.4L9.3 15.4c.39.4 1.02.4 1.41 0l5.67-5.63a1 1 0 000-1.42z" clipRule="evenodd" />
                                                            </svg>
